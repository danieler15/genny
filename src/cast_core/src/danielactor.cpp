...
#include <cast_core/actors/danielactor.hpp>
...
// them to the `_loop{}` initializer in the `danielactor` constructor
...
struct danielactor::PhaseConfig {
...
void danielactor::run() {
    ...
    for (auto&& config : _loop) {
        ...
        for (const auto&& _ : config) {
            ...
            // debug level.
            //
            BOOST_LOG_TRIVIAL(debug) << " danielactor Inserting "
...
danielactor::danielactor(genny::ActorContext& context)
    : Actor{context},
      _totalInserts{context.operation("Insert", danielactor::id())},
      ...
      //
      _loop{context, (*_client)[context["Database"].to<std::string>()], danielactor::id()} {}
...
// in the header file.
//
auto registerdanielactor = Cast::registerDefault<danielactor>();
}  // namespace
}  // namespace genny::actor
